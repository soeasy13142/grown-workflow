# Skill 整理方法论 v0 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把 Skill 整理方法论 v0 沉淀到 `docs/methodology.md`（独立 markdown），作为本项目 v0 可执行的方法。

**Architecture:** 单文件 `docs/methodology.md`——4 阶段流程、SKILL.md 模板、6 个避坑反模式、4 个升级触发条件、v0 已知不足段。独立于 CLAUDE.md（方法论本身可独立迭代，与项目主契约分离）。

**Tech Stack:** Markdown, Git。

## Global Constraints

- **文件位置：** `docs/methodology.md`（仓库根 `docs/` 下，与 `principles.md` 平级）
- **独立于 CLAUDE.md：** 方法论不写入 CLAUDE.md 任何节（CLAUDE.md §0 决策原则已通过 Stage 2 的 3 个问题实例化）
- **v0 诚实姿态：** 文档末尾必须含「v0 已知不足」段（方法论自身的不足）
- **不预定义 schema：** SKILL.md 模板字段全部可选，v0 自由
- **不引入状态机：** skill 生命周期不强制 seed → draft → curated → verified → deprecated
- **语言：** 中文（与 README、CLAUDE.md 一致）
- **不 push：** 本计划只生成 1 个本地 commit
...
各文件职责：
- `docs/methodology.md` — Skill 整理方法论 v0 文档，4 阶段流程 + 模板 + 反模式 + 升级触发
